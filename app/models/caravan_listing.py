...
class CaravanListingCreate(BaseModel):
    ...
    )  # toilet, shower, kitchenette, fridge, heating, AC, solar, awning, bikeRack, storageBoxes
    power_source: List[str] | None = Field(
        default_factory=list
    )  # battery, shore_power, solar, generator
    water_system: Annotated[str, Field(max_length=200)] | None = (
        None  # e.g. "fresh_tank: 100L, grey_tank: 80L"
    )
