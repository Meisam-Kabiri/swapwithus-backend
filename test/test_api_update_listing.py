...
async def test_update_listing_home(create_db_pool):
    ...
    image_metadata[1]["public_url"] = (
        public_url_to_keep if public_url_to_keep else None
    )  # Keep first image
    ...
    with (
        ...
        patch("app.api.listings.extract_firebase_user_uid", return_value=owner_uid),
    ):
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
            ...
            print(response.json())
            public_urls_on_database_after = await create_db_pool.fetch(
                "SELECT public_url FROM images WHERE listing_id = $1 ORDER BY sort_order",
                listing_id,
            )
            number_of_images_after_on_table = await create_db_pool.fetchval(
                "SELECT COUNT(*) FROM images WHERE listing_id = $1", listing_id
            )
            number_of_test_images_on_gcp_after, images_list_on_gcp_after = (
                number_of_test_images_in_gcp()
            )
            assert response.status_code == 200
            assert number_of_homelistings_before == await create_db_pool.fetchval(
                "SELECT COUNT(*) FROM homes"
            )
            assert (
                number_of_images_after_on_table
                == len(public_urls_on_database_before) + num_of_new_images - 2
            )  # -2 means out of #num_new_image one image was alraedy there and one images was deleted
            assert public_url_to_keep in [
                dict(url).get("public_url") for url in public_urls_on_database_after
            ]
            assert public_url_to_delete not in [
                url["public_url"] for url in public_urls_on_database_after
            ]
            assert (
                number_of_test_images_on_gcp_after
                == number_of_test_images_on_gcp_before + num_of_new_images - 2
            )
            assert any(file_name in public_url_to_keep for file_name in images_list_on_gcp_after)
            assert all(
                file_name not in public_url_to_delete for file_name in images_list_on_gcp_after
            )
